...
def make_gaussian_mask(height, width):
    ...
    mask = np.full((height, width), value)
    mask[half_height_index, :] += np.full_like(mask[half_height_index, :], value)
    mask[:, half_width_index] += np.full_like(mask[:, half_width_index], value)
    return mask
